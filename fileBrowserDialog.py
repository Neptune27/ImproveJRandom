...
class SaveDialog(QFileDialog):
    def __init__(self, texts, heading=None):
        if heading is None:
            heading = ''
        super().__init__()
        save_location, _ = self.getSaveFileName(self, "Save", "", "Text file (*.txt) ;;Word Document (*.docx)")
        try:
            if save_location.endswith(".txt"):
                with open(save_location, "w", encoding='UTF-8') as save:
                    save.write(texts)
            elif save_location.endswith(".docx"):
                # self.initDocxOptions()
                self.initDocxOptions()

                if self.docxOption.isAccepted:
                    wordObjList = SQLController.getWordsObjects(texts)
                    # print(save_location)
                    self.document = docx.Document()
                    docHeading = self.document.add_heading()
                    run = docHeading.add_run(f'{heading}')
                    font = run.font
                    font.size = Pt(35)
                    ...
                    self.document.core_properties.author = 'Kanji Browser by Minh Trí'
                    self.document.save(f'{save_location}')
            else:
                self.initWarningBox('Please select correct file!', 'Warning')
        except FileNotFoundError:
            print("[WARNING] File not found: " + save_location)
        except PermissionError:
            self.initWarningBox(f'Permission denied. Please close any app associated \nwith this file and try again.',
                                'Error')

    def initWarningBox(self, text, title):
        self.warning = QMessageBox()
        self.warning.setText(text)
        self.warning.setWindowTitle(title)
